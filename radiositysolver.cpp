...
{
	double x, y;
	double da = 2.0 * 2.0 / (numPixelsOnWidth * numPixelsOnWidth);
	double pixelWidth = 2.0 / numPixelsOnWidth;
	double pixelHeight = pixelWidth;

	double initialX = -1 + pixelWidth / 2.0;
    double initialY = initialX;
    
	for (int row = 0; row < numPixelsOnWidth; row+=1) {
		y = initialY + (double)row * pixelHeight;

		for (int col = 0; col < numPixelsOnWidth; col+=1) {
			x = initialX + (double)col * pixelWidth;
			deltaFormFactors[numPixelsOnWidth * row + col] = da / (M_PI * pow(x*x + y*y + 1, 2));
		}
	}
}
    ...
    // Note that numPixelsOnWidth must be a even number.
{
	...
	double initialY = 1 - pixelWidth / 2.0;
	double initialZ = 0 + pixelWidth / 2.0;

	for (int row = 0; row < numPixelsOnWidth / 2; row++) {
		z = initialZ + (double)row *  pixelHeight;

		for (int col = 0; col < numPixelsOnWidth; col++) {
			y = initialY - (double)col * pixelWidth;
			deltaFormFactors[numPixelsOnWidth * row + col] = da * z / (M_PI * pow(y*y + z*z + 1, 2));
		}
	}
}
    ...
    // Need to set up the viewport, projection and view transfromation.
{
	...
	float lookUpVec[3];
	float sideVec[3];
	float sideNormalVec[3];

	VecDiff(sideVec, shooterQuad->v[0], shooterQuad -> v[1]);
	VecNormalize(sideNormalVec, sideVec);
	VecCrossProd(lookUpVec, sideNormalVec, shooterQuad->normal);

	gluLookAt(shooterQuad->centroid[0], 
				shooterQuad->centroid[1], 
				shooterQuad->centroid[2], 
    ...
    // Need to set up the viewport, projection and view transfromation.
{
	...
	switch (face) {
	case 1:
		VecCrossProd(lookDir, sideVec, shooterQuad->normal);
		break;
	...
	case 3:
		VecCrossProd(lookDirTemp, sideVec, shooterQuad->normal);
		VecNeg(lookDir, lookDirTemp);
		break;

	case 4:
		lookDirTemp[0] = sideVec[0];
		lookDirTemp[1] = sideVec[1];
		lookDirTemp[2] = sideVec[2];
	...
	gluLookAt(shooterQuad->centroid[0],
		shooterQuad->centroid[1],
		shooterQuad->centroid[2],
		shooterQuad->centroid[0] + lookDir[0],
		shooterQuad->centroid[1] + lookDir[1],
		shooterQuad->centroid[2] + lookDir[2],
		shooterQuad->normal[0],
		shooterQuad->normal[1],
		shooterQuad->normal[2]);
}
    ...
    // and update the unshot power of their parent shooter quads.
{
    for ( int i = 0; i < width * height; i++ )
    {
		...
		float powerR, powerG, powerB;
		powerR = m->gatherers[g]->surface->reflectivity[0] * deltaFormFactors[i] * shotPower[0];
		powerG = m->gatherers[g]->surface->reflectivity[1] * deltaFormFactors[i] * shotPower[1];
		powerB = m->gatherers[g]->surface->reflectivity[2] * deltaFormFactors[i] * shotPower[2];

		m->gatherers[g]->radiosity[0] += powerR / m->gatherers[g]->area;
		m->gatherers[g]->radiosity[1] += powerG / m->gatherers[g]->area;
		m->gatherers[g]->radiosity[2] += powerB / m->gatherers[g]->area;

		m->gatherers[g]->shooter->unshotPower[0] += powerR;
		m->gatherers[g]->shooter->unshotPower[1] += powerG;
		m->gatherers[g]->shooter->unshotPower[2] += powerB;
    }
}
